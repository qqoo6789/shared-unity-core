...
public static class HomeSkillCastHelpUtilCore
{
    ...
    /// <returns></returns>
    public static (float damage, bool isCrit) CalculateSegmentProgressDamage(EntityBase fromEntity, ICollectResourceCore targetObject, eAction targetCurAction, DRSkill drSkill, InputRandomData seedData = null)
    {
        float damage;
        bool isCrit = false;
        ...
        else
        {
            ...
            (float calculateDamage, bool crit) = SkillDamage.CalculateHomeDamage(targetCurAction, fromEntity.EntityAttributeData, toAttribute, drSkill.HomeAttRate * TableDefine.THOUSANDTH_2_FLOAT, seedData);
